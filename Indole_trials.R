...
#combine datasets
indole.d <- rbind(d1.1, d2.1, d3.1, d4.1, d5.1, d1.2, d2.2, d3.2, d4.2, d5.2)
indole.c <- rbind(c1.1, c2.1, c3.1, c4.1, c5.1, c1.2, c2.2, c3.2, c4.2, c5.2)
...
indole.plot1<-ggplot(data=all.in.ag, aes(x=treatment, y=FILES))+ 
	...
	theme_classic()+
	labs(x=" ", y="Relative activity (no. nightly recordings)", title = "Indole trials")+
...
#simple graph
ggplot(data=all.in.ag, aes(x=treatment, y=FILES))+ 
	stat_summary(fun.data = "mean_se", colour="black", size=1.5, shape="diamond")+
	theme_classic()+
	labs(x=" ", y="Relative activity (no. nightly recordings)", title="Indole trials")+
	theme(text = element_text(size=16), legend.title = )+
...
library(lme4)
library(car)
indole.mod1<-glmer(FILES~treatment*AUTO.ID.+1|plot, dat = all.in.ag)
summary(indole.mod1)
...
summary(indole.mod2)
Anova(indole.mod2)
#everything significant, but doesn't account for plot or date...
...
ggplot(data=bigbrown.indole, aes(x=treatment, y=FILES))+ 
	...
	labs(x=" ", y="Relative activity (no. nightly recordings)",
		 title = "Big brown bats (EPTFUS), Indole")+
...
indole.mod.bb<-glmer(FILES~treatment + 1|plot, data = bigbrown.indole)
